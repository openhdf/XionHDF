...
class XionHDF(ConfigListScreen, Screen):
	...
	<screen name="XionHDF-Setup" position="0,0" size="1280,720" flags="wfNoBorder" backgroundColor="#00000000">
		...
		<eLabel position="845,130" size="372,46" text="Version: 1.7" font="Regular; 30" valign="center" halign="center" transparent="1" backgroundColor="#00000000" foregroundColor="#00ffffff" name="," />
	...
	def checkWeather(self):
		if config.plugins.XionHDF.WeatherStyle.value in ("weather-big", "weather-slim", "weather-small"):
			if self.searchString != config.plugins.XionHDF.weather_cityname.value:
	...
	def setWeather(self):
		setweather = self.session.openWithCallback(self.doSetWeather, MessageBox, _("The 'Weather ID' was changed.\nDo you want to apply the new ID?"), MessageBox.TYPE_YESNO)
		setweather.setTitle(_("Weather ID changed"))
	...
	def getWeatherData(self):
		if config.plugins.XionHDF.weather_cityname.value == "":
			...
			reslist = []
			try:
				res = requests.get('http://dev.virtualearth.net/REST/v1/Locations/' + self.searchString + '?&key=AiU2Jrx506GVhCwH5kw6h6KLiYGbgwWDtYUFvDxYNYQ5yAcb81_RxlIMmFOLB8Rr', timeout=3)
				data = res.json()
			...
			if len(reslist) > 1:
				...
				config.plugins.XionHDF.weather_latitude.value = str(lat)
